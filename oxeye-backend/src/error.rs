            ...
            AppError::DatabaseError(db_err) => {
                ...
                let (status, message) = match db_err {
                    oxeye_db::DbError::PendingLinkNotFound => {
                        (StatusCode::NOT_FOUND, "Connection code not found or expired")
                    }
                    oxeye_db::DbError::PendingLinkAlreadyUsed => {
                        (StatusCode::CONFLICT, "Connection code has already been used")
                    }
                    oxeye_db::DbError::ServerNameConflict => {
                        (StatusCode::CONFLICT, "A server with this name already exists")
                    }
