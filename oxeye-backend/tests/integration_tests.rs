...
async fn test_connect_success() {
    ...
    // THEN: Should return 200 OK with an API key
    assert_eq!(status, StatusCode::OK);
...
async fn test_multiple_servers_in_same_guild() {
    ...
    let players1 = db.get_online_players(hash1).await.expect("Failed to get players");
    assert_eq!(players1.len(), 1);
    assert_eq!(players1[0], "Steve");

    let hash2 = helpers::hash_api_key(&api_key2);
    let players2 = db.get_online_players(hash2).await.expect("Failed to get players");
