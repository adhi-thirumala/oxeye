...
#[derive(Clone)] // only for testing - never happens in production because wrapped in an arc
// anyways
pub struct Database {
    conn: Connection,
}
...
impl Database {
    ...
    /// Get a specific server by guild ID and name.
    pub async fn get_server_by_guild_and_name(
        &self,
        guild_id: u64,
        name: &str,
        ...
        let server = self
            ...
            .call(move |conn| {
                let mut stmt = conn.prepare_cached(
                    "SELECT api_key_hash, name, guild_id FROM servers WHERE guild_id = ?1 AND name = ?2",
                )?;

                let server = stmt
                    .query_row(params![guild_id, name], |row| {
                        Ok(Server {
                            api_key_hash: row.get(0)?,
                            name: row.get(1)?,
                            guild_id: row.get(2)?,
